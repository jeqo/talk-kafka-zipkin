...
public class HelloBatchClient {
  ...
  public static void main(String[] args) {
    ...
    final WebClient httpClient = WebClient
      .builder(config.getString("hello-service.base-url"))
      .decorator(
        BraveClient.newDecorator(httpTracing.clientOf("hello-service"))
      )
      .build();
    ...
    final var kafkaProducer = new KafkaProducer<String, String>(
      producerConfigs
    );
    ...
    try (Tracer.SpanInScope ignored = tracer.withSpanInScope(batchSpan)) {
      ...
      for (String name : names) {
        ...
        try (Tracer.SpanInScope ws = tracer.withSpanInScope(span)) {
          ...
          tracedKafkaProducer.send(
            new ProducerRecord<>("hello", hello),
            (metadata, exception) -> {
